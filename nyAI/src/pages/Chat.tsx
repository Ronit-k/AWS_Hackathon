...
const Chat = () => {
  ...
  const sendMessage = (text: string) => {
    ...
    // Simulated bot response (replace with real AI later)
    setTimeout(() => {
      const botMsg: Message = {
        id: (Date.now() + 1).toString(),
        role: "assistant",
        content:
          "Thank you for your question. This is a demo response — when connected to our AI backend, I'll provide detailed legal guidance based on Indian law including the Constitution, IPC, and relevant acts.\n\nI would explain your rights clearly, suggest practical next steps, and provide emergency contacts if needed. Your privacy is always protected.",
      };
    ...
    <Layout>
      <div className="container max-w-3xl py-6 flex flex-col" style={{ minHeight: "calc(100vh - 12rem)" }}>
        ...
        <form onSubmit={handleSubmit} className="flex items-center gap-2 bg-card border border-border rounded-2xl px-3 py-2 shadow-sm">
          ...
          />
          <Button type="submit" size="icon" variant="default" disabled={!input.trim()} className="rounded-xl">
